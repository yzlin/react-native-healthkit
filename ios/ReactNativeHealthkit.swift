...
class ReactNativeHealthkit: RCTEventEmitter {
    ...
    @objc(queryStatisticsCollectionForQuantity:unitString:from:to:options:anchorDate:interval:resolve:reject:)
    func queryStatisticsCollectionForQuantity(typeIdentifier: String, unitString: String, from: Date, to: Date, options: NSArray, anchorDate: Date, interval: NSDictionary, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
        guard let store = _store else {
            return reject(INIT_ERROR, INIT_ERROR_MESSAGE, nil)
        }
        ...
        let query = HKStatisticsCollectionQuery(quantityType: quantityType, quantitySamplePredicate: predicate, options: opts, anchorDate: anchorDate, intervalComponents: intervalComponents)
        ...
        query.initialResultsHandler = { query, statsCollection, error in
            ...
            for gottenStats in statsArray {
                ...
                if let averageQuantity = gottenStats.averageQuantity() {
                    dic.updateValue(serializeStatistic(unit: unit, quantity: averageQuantity, stats: gottenStats), forKey: "averageQuantity")
                }
                if let maximumQuantity = gottenStats.maximumQuantity() {
                    dic.updateValue(serializeStatistic(unit: unit, quantity: maximumQuantity, stats: gottenStats), forKey: "maximumQuantity")
                }
                if let minimumQuantity = gottenStats.minimumQuantity() {
                    dic.updateValue(serializeStatistic(unit: unit, quantity: minimumQuantity, stats: gottenStats), forKey: "minimumQuantity")
                }
                if let sumQuantity = gottenStats.sumQuantity() {
                ...
                if #available(iOS 12, *) {
                    if let mostRecent = gottenStats.mostRecentQuantity() {
                        dic.updateValue(serializeStatistic(unit: unit, quantity: mostRecent, stats: gottenStats), forKey: "mostRecentQuantity")
                    }
